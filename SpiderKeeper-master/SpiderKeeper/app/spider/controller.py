...
def StartJdSpider():
    ...
    if url=="":
        return "请输入url,url不能为空!"
    # print(url)
    r = CRedis()
    r.lpush('jd:start_urls', url)
    return "url已经加入redis队列！"
...
def info():
        ...
        for infor in infors:
             results.append(
                {'title': infor[1], 'price': infor[3], 'evaluate': infor[6], 'store': infor[4]})
        ...
        result=[]
        result.append(
            {'count': count, 'num_pages': num_pages, 'curr_page': curr_page, 'previous_page_number': previous_page_number, 'nex_page_number': nex_page_number, 'last_page': last_page, 'has_previous': has_previous, 'has_next': has_next})
        return render_template('info.html', results=results, result=result)
...
def getBlog():
    ...
    result = []
    result.append(
        {'count': count, 'num_pages': num_pages, 'curr_page': curr_page, 'previous_page_number': previous_page_number,
         'nex_page_number': nex_page_number, 'last_page': last_page, 'has_previous': has_previous,
         'has_next': has_next})
    return render_template('blog.html', results=results, result=result)
